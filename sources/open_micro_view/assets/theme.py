...
def configure_style(master) -> ThemedStyle:
    ...
    # BUTTONS
    gui_style.configure('TButton', font=('Quicksand', 18), background="#FEFEFE", bordercolor='darkgrey',
                        borderwidth=1, relief=FLAT, padding=5)
    gui_style.configure('close.TButton', font=('Quicksand', 18), background="#EFEEEE", relief=FLAT,
                        padding=5)
    gui_style.configure('control.TButton', font=('Quicksand', 24), background="#FFFFFF", relief=FLAT,
                        padding=5)
    gui_style.configure('config.TButton', font=('Quicksand', 12), background="#FEFEFE", relief=FLAT,
                        padding=3)
    gui_style.configure('time.TButton', font=('Quicksand', 15), background="#FFFFFF", relief=FLAT,
                        padding=5)
    gui_style.configure('del.TButton', font=('Quicksand', 15), background="#FFDDDD", relief=FLAT,
                        padding=2)
    # NOTEBOOK
    # gui_style.layout("TNotebook", [])
    gui_style.configure('TNotebook.Tab', font=('Quicksand', 15), background="lightgray", padding=5,
                        bordercolor='#FF0000')
    gui_style.map('TNotebook.Tab', background=[('selected', 'white')])
    gui_style.configure('TNotebook', padding=5, tabmargins=0)
    # CHECKBOXES / RADIO BUTTONS
    gui_style.configure('TCheckbutton', font=('Quicksand', 12), background="white", relief=FLAT,
                        indicatorrelief=FLAT, indicatormargin=3, padding=10)
    gui_style.configure('TRadiobutton', font=('Quicksand', 12), background="white", relief=FLAT,
                        indicatorrelief=FLAT, indicatormargin=3, padding=10)
    # TSCALE
    gui_style.configure('Horizontal.TScale', font=('Quicksand', 20), sliderthickness=30, relief=FLAT,
                        padding=10)
    gui_style.configure('Horizontal.Player.TScale', font=('Quicksand', 12), sliderthickness=15,
