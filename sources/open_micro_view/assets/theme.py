...
def configure_style(master) -> ThemedStyle:
    ...
    gui_style.configure('TButton', font=(FONT_NAME, 18), background="#FEFEFE", bordercolor='darkgrey', borderwidth=1, relief=FLAT, padding=5) # inside padding
    gui_style.configure('close.TButton', font=(FONT_NAME, 18), background="#EFEEEE", relief=FLAT, padding=5) # inside padding
    ...
    gui_style.configure('del.TButton', font=(FONT_NAME, 15), background="#FFDDDD", relief=FLAT, padding=2) # delete Button
    gui_style.configure('shutdown.TButton', font=(FONT_NAME, 12), background="#FFDDDD", relief=FLAT, padding=3)

    # NOTEBOOK
    # gui_style.layout("TNotebook", [])
    gui_style.configure('TNotebook.Tab', font=(FONT_NAME, 15), background="lightgray", padding=5, bordercolor='#FF0000')
    gui_style.map('TNotebook.Tab', background=[('selected', 'white')] )
    gui_style.configure('TNotebook', padding=5, tabmargins=0)
    # CHECKBOXES / RADIO BUTTONS
    gui_style.configure('TCheckbutton', font=(FONT_NAME, 12), background="white", relief=FLAT, indicatorrelief=FLAT, indicatormargin=3, padding=10)
    gui_style.configure('TRadiobutton', font=(FONT_NAME, 12), background="white", relief=FLAT, indicatorrelief=FLAT, indicatormargin=3, padding=10)
    # TSCALE
