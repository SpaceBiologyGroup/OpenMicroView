...
from .utils import B_to_readable, create_popup, create_progress_popup, dir_size_bytes, umount2
...
#Class :     Settings
#Params:     1: microscope <Microscope>
#            2: app <App>
class Settings:
    def __init__(self, microscope, app):
        ...
        self.frame      = None
        self.app        = app # To call hideSettings
    ...
    def initPanel(self, frame:Frame):
        ...
        ttk.Label(frame, text='Settings', style='title.TLabel').grid(column=0,
                                                                     ...
                                                                     sticky='nsew')
        # Exit Button
        ttk.Button(frame, text="Back", command=self.app.hideSettings,
        ...
        browse_btn = ttk.Button(self.tab_details, text="Browse Pictures", style='TButton', command=self.image_browser)
    ...
    def selectResolution(self, r):
        ...
        r = round(float(r))
        #modes={'0':1,'1':5,'2':4,'3':2}
        res = {0:(800, 480), 1:(1296, 730), 2:(1296, 972),
               3:(1920, 1080), 4:(2592, 1944), 5:(3280, 2464)}
        resolutions = { k: f"{x}x{y} ({int(x/gcd(x, y))}:{int(y/gcd(x, y))}) " for k, (x, y) in res.items()}
    ...
    #Method :     saveConfig
    #Parameters:  none
    #Description: Get current config and save it to CONFIG_FILE
    def saveConfig(self):
    ...
    #Method :     loadConfig()
    #Parameters:  none
    #Description: Load Config from CONFIG_FILE and apply it
    def loadConfig(self):
    ...
    #Method :     getConfig()
    #Parameters:  none
    #Description: return current config in a dictionnary
    def getConfig(self):
        return ({
    ...
    def refreshDevicesList(self):
        ...
        if self.frame_cp != None: self.frame_cp.destroy()
    ...
    def delete_pictures(self, popup):
        ...
        files = [f for f in os.listdir(path)
                    if f.split('.')[-1] in ['jpeg', 'jpg', 'png']]
        total = len(files)
        progress = IntVar(0)
        status = StringVar("")
        i = 0
        if total <= 0:
            logging.warning(f'No Files to delete.')
            return None
    ...
    def eject_usb(self):
        ...
        try:
            ...
            create_popup("Cancel", f"Impossible to eject device, try again later:\n Error: {os.strerror(e.errno)}")    
