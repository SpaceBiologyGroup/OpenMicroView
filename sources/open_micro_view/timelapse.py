...
class Timelapse:
    ...
    def initTimelapseTab(self, container):
        ...
        # LINE 0
        #ttk.Label(tab, text='Interval:').grid(column=0, row=0, columnspan=2, sticky='news')
        ...
        ttk.Label(tab, textvar=self.value,
                  style='tlvalue.TLabel').grid(column=0, row=1, sticky='nes', padx=5, pady=5)
        self.value.set(0)
        ttk.Button(tab, text='-',
                   style='control.TButton',
                   command=self.valueMinus).grid(column=1, row=1, sticky='news', padx=5, pady=5)
        self.plus= ttk.Button(tab, text='+',
                              style='control.TButton',
                              command=self.valuePlus).grid(column=2, row=1, sticky='news', padx=5, pady=5)
        # LINE 3
    ...
    def stopTimelapse(self):
        ...
        self.root_app.timelapseStopped()
        ...
        while (True):
            ...
            if (last is None or (now - begin).total_seconds() >= interval * qt_photos ):
                ...
                try:
                    ...
                    logging.error(f"Impossible to capture picture {filename}", exc_info=True)
            ...
            if interval > MIN_INTERVAL_AUTOLIGHT:
                if ((n <= AUTOLIGHT_INTERVAL and self.light_status == 0)
                    or (n > interval - 5 and self.light_status == 1)):
                    self.toggleLight()
